...
export class SistemaService {
  private pathBase: string = "/api/usuarios/sistema/v1";
  ...
  public update(idSistema: number, model: any) {
    return this.http.put(`${env.HOST}${this.pathBase}/${idSistema}`, model).pipe(take(1));
  }

  public delete(idSistema: number) {
    return this.http.delete(`${env.HOST}${this.pathBase}/${idSistema}`).pipe(take(1));
  }

  public findById(idSistema: number) {
    return this.http.get<any>(`${env.HOST}${this.pathBase}/${idSistema}`).pipe(take(1));
  }
