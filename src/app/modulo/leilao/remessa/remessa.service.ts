...
export class RemessaService {
  private pathBase: string = "/api/leilao/remessa/v1";
  ...
  public update(idRemessa: number, model: any) {
    return this.http.put(`${env.HOST}${this.pathBase}/${idRemessa}`, model).pipe(take(1));
  }

  public delete(idRemessa: number) {
    return this.http.delete(`${env.HOST}${this.pathBase}/${idRemessa}`).pipe(take(1));
  }

  public findById(idRemessa: number) {
    return this.http.get<any>(`${env.HOST}${this.pathBase}/${idRemessa}`).pipe(take(1));
  }
  ...
  public findAll(pagina: number, qtd: number, campo: string, ordem: number, filtro: string) {
    ...
    if (filtro != '') {
      if (campo == 'id') {
        ...
        aplicandoFiltro = '/search-ano/' + filtro
      }
    }
